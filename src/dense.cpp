...
void init_dense(py::module_ &module_matrix) {
    ...
    auto init_func = [](std::shared_ptr<gko::Executor> exec, py::buffer b) {
        ...
        auto elems = (info.ndim == 1) ? info.shape[0] : info.shape[0] * info.shape[1];

        auto view = gko::array<ValueType>::view(
            exec->get_master(), elems, (ValueType *)info.ptr);
    ...
    py::class_<gko::matrix::Dense<ValueType>, std::shared_ptr<gko::matrix::Dense<ValueType>>, gko::LinOp>(
        ...
        .def(py::init(
            [init_func](std::shared_ptr<gko::Executor> exec, py::buffer b) { 
        ...
        .def(py::init(
            [](std::shared_ptr<gko::Executor> exec, py::tuple dim, IndexType stride) {
                return gko::share(gko::matrix::Dense<ValueType>::create(
                    exec, gko::dim<2>{dim[0].cast<gko::size_type>(), dim[1].cast<gko::size_type>()}, stride));
        }))

        .def(py::init(
            [](std::shared_ptr<gko::Executor> exec, py::tuple dim, py::buffer b, size_t stride) {
                py::buffer_info info = b.request();
                if (info.format != py::format_descriptor<ValueType>::format())
                    throw std::runtime_error("Incompatible dtype");
        ...
        .def(py::init(
            [](std::shared_ptr<gko::Executor> exec, py::tuple dim, gko::array<ValueType> view, size_t stride) {
                return gko::share(gko::matrix::Dense<ValueType>::create(
                    exec, gko::dim<2>{dim[0].cast<size_t>(), dim[1].cast<size_t>()},
                    view, stride));
            }))
        ...
        .def_buffer([](gko::matrix::Dense<ValueType> &m) -> py::buffer_info {
            ...
            if (dim == 1) {
                ...
                return py::buffer_info(
                    ...
                    {sizeof(ValueType), sizeof(ValueType) * rows} /* Strides (in bytes) for each index */
                );
            }
        })
